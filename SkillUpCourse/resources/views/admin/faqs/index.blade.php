<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center bg-indigo-100 px-4 py-3 rounded-md shadow-md">
            <h2 class="text-2xl font-semibold text-indigo-900 leading-tight">Manage FAQs</h2>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10">
                    ...
                    <table id="faqTable" class="table-auto w-full">
                        ...
                        <tbody>
                            @forelse ($faqs as $index => $faq)
                                <tr class="border-b">
                                    <td class="px-4 py-2">
                                        {{ ($faqs->currentPage() - 1) * $faqs->perPage() + $index + 1 }}</td>
                                    <td class="px-4 py-2">{{ $faq->question }}</td>
                                    <td class="px-4 py-2">{{ $faq->created_at }}</td>
                                    <td class="px-4 py-2">
                                        <div class="ml-auto flex space-x-3">
                                            ...
                                            <!-- Tambahkan tautan untuk melihat detail FAQ -->
                                            <a href="#"
                                                onclick="showFAQDetailModal('{{ $faq->question }}', '{{ $faq->answer }}')"
                ...
                <div class="mt-4 flex justify-center">
                    {{ $faqs->appends(['search' => request('search')])->links('vendors.pagination.custom') }}
                </div>
            </div>
        </div>
    </div>

    <!-- Markup untuk modal detail FAQ -->
    <div id="faqDetailModal" class="fixed z-10 inset-0 overflow-y-auto hidden">
        <div class="flex items-center justify-center min-h-screen">
            <div class="bg-white p-6 rounded-lg shadow-lg max-w-md mx-auto">
                <div class="mb-4">
                    <h3 class="text-lg font-semibold">FAQ Details</h3>
                </div>
                <div class="mb-4">
                    <p id="faqDetailQuestion" class="text-gray-700"></p>
                </div>
                <div class="mb-4">
                    <p id="faqDetailAnswer" class="text-gray-700"></p>
                </div>
